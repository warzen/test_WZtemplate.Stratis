...
// Example: _plane,_pilotGroup,_pilot = [_planeType,  _spawn, _pos, _flyHight, _captive] call MCC_fnc_createPlane; //exemple faux, ne retourne plus que le véhicule
...
// exemple : ["rhs_ka60_grey", getMarkerPos "depart", getMarkerPos "renfort", 100, false, EAST] call WZ_fnc_createPlane;
...
_vehicleClass 	=  getText (configFile >> "CfgVehicles" >> _planeType >> "vehicleClass");  // pour les hélico RHS, c'est "rhs_vehclass_helicopter"
...
if !(_plane isKindOf "Helicopter") then		// il faudrait vérifier que cela marche avec les hélico rhs - normalement oui d'après la définition des classes RHS
{
	...
	// set marker on map	// suppression du marker
